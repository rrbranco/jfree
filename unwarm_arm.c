...
UnwResult UnwStartArm(UnwState * const state)
{
    ...
    do
    {
        ...
        //if (state->regData[15].v >= 0x41567000 && state->regData[15].v < 0x41569000)
        //if ((instr & 0x0000f000)== 0x0000d000)
        //  fprintf(stderr, "A %x %x %08x %x:\n", state->regData[13].v, state->regData[15].v, instr, t);
         ...
         */
        if((instr & 0xffffffd0) == 0xe12fff10)
        {
            Boolean     L       =   (instr & 0x00000020);
            if (!L)
            {
                Int8 rn = instr & 0xf;
        ...
        /* Branch */
        else if((instr & 0xfe000000) == 0xea000000)
        {
            Boolean     L       =   (instr & 0x01000000);
            if (!L)
            {
        ...
        else if((instr & 0xfe000000) == 0xe8000000)
        {
            ...
            SignedInt8 r;
/*
            if (instr == 0xe8bd87f0 && state->regData[15].v == 0x410cf15c)
            {
                Int32 stckval;
                int i = 0;
                while (i < 8)
                {
                    state->cb->readW(state->regData[13].v+(i*4), &stckval);
                    fprintf(stderr, "0x%08x ", stckval);
                    i++;
                }
                fprintf(stderr, "\n");
            }
*/
            ...
            do
            {
                ...
                if(regList & (0x01 << r))
                {
                    ...
                    else
                    {
                        if(addrValid)
                        {
                            if(!UnwMemWriteRegister(state, addr, &state->regData[r]))
                            {
                                return UNWIND_DWRITE_W_FAIL;
                            }
                        }

                        //fprintf(stderr, "addr: 0x%08x value: 0x%08x\n", addr, state->regData[r].v);
        ...
        /* LDR handling for a few very specific situations (PLT) */
        else if(((instr & 0xfc100000) == 0xe4100000)
                && ((state->regData[15].v >= 0x40000000 && state->regData[15].v < 0x40004000)
                    || ((instr & 0x0000f000) == 0x0000f000)))
        {
            Boolean     P           = (instr & 0x01000000) ? TRUE : FALSE;
            Boolean     W           = (instr & 0x00200000) ? TRUE : FALSE;
            Boolean     R           = (instr & 0x02000000) ? TRUE : FALSE;
            Boolean     U           = (instr & 0x00800000) ? TRUE : FALSE;
            Int16       destReg     = (instr & 0x0000f000) >> 12;
            Int16       baseReg     = (instr & 0x000f0000) >> 16;
            Int32       addr        = state->regData[baseReg].v;
            Boolean     addrValid   = M_IsOriginValid(state->regData[baseReg].o);

            //fprintf(stderr, "%d %d %d %d %d %d 0x%08x %d\n", P, W, R, U, destReg, baseReg, addr, addrValid);

            /* Simple register offset (no embedded shift) */
            if (R && !(instr & 0x00000ff0))
            {
                Int16 rm = (instr & 0x0000000f);
                
                if (M_IsOriginValid(state->regData[rm].o))
                    if (U)
                        addr += state->regData[rm].v;
                    else
                        addr -= state->regData[rm].v;
                else
                    addrValid = FALSE;
            ...
            if (addrValid)
            {
                ...
                if(!UnwMemReadRegister(state, addr, &state->regData[destReg]))
                    return UNWIND_DREAD_W_FAIL;

                state->regData[destReg].o = REG_VAL_FROM_MEMORY;
                //fprintf(stderr, "0x%08x\n", state->regData[destReg].v);

                if (destReg == 15)
                {
                    /* Account for the auto-increment which isn't needed */
                    state->regData[destReg].v -= 4;
                }
            ...
            {
                state->regData[destReg].o = REG_VAL_INVALID;
                if (P && W)
                    state->regData[baseReg].o = REG_VAL_INVALID;
            }
        }
        else
        {
            ...
            /* Trying to guess if it changes SP, LR or PC too */
            Boolean     coProc  =   ((instr & 0x0e000000) == 0x0c000000) ? TRUE : FALSE;
            Boolean     unCond  =   ((instr & 0xf0000000) == 0xe0000000) ? TRUE : FALSE;
            Boolean     P       =   (instr & 0x01000000) ? TRUE : FALSE;
            Boolean     W       =   (instr & 0x00200000) ? TRUE : FALSE;
            Int16       baseReg =   (instr & 0x000f0000) >> 16;
            Int8        destReg =   (instr & 0x0000f000) >> 12;

            if (unCond)
            {
                if ( P && W)
                {
                    if (baseReg == 13 || baseReg == 14 || baseReg == 15)
                    {
                        state->regData[baseReg].o = REG_VAL_INVALID;
                        //fprintf(stderr, "instr: %08x baseReg: %d PC: %08x\n", instr, baseReg, state->regData[15].v);
                    }
                }
                if (!coProc && ((destReg == 13 || destReg == 14 || destReg == 15)))
                {
                    state->regData[destReg].o = REG_VAL_INVALID;
                    //fprintf(stderr, "instr: %08x destReg: %d PC: %08x\n", instr, destReg, state->regData[15].v);
                }
            }
        }
        ...
        //if ((state->regData[15].v & 0xbeff0000) == 0xbeff0000)
        {
        //fprintf(stderr, "instr: 0x%08x\n", instr);
        //fprintf(stderr, "PC: 0x%08x\n", state->regData[15].v);
        }
